...
func checksum(data []byte) (uint8, uint8) {
	crc := uint16(0xFFFF)
...
func (d *Door) GetRecordInformation() {
	info := []byte{ANVIZ, 0x00, 0x00, 0x00, 0x01, 0x3C, 0x00, 0x00}
	...
	data := response[9:28]
	user_count := data[1:3]
	log.Printf("user amount: %n\n", user_count)
}

func (d *Door) DownloadStaffInformation() {
	info := []byte{ANVIZ, 0x00, 0x00, 0x00, 0x01, 0x42, 0x00, 0x02, 0x00, 0x09}
	l, h := checksum(info)
...
func (d *Door) GetInformation() {
	info := []byte{ANVIZ, 0x00, 0x00, 0x00, 0x01, 0x30, 0x00, 0x00}
	l, h := checksum(info)
